...
function ContactContent() {
  ...
  useEffect(() => {
    ...
    if (booking === 'true') {
      setFormData(prev => ({ ...prev, subject: 'landscape' }))
    } else if (service) {
      if (service === 'Garden Care') {
        setFormData(prev => ({ ...prev, subject: 'care' }))
      } else if (service === 'Landscape Design' || service === 'Plant Rental') {
        setFormData(prev => ({ ...prev, subject: 'landscape' }))
      } else if (service === 'Nursery Visits') {
        setFormData(prev => ({ ...prev, subject: 'wholesale' }))
      }
    }
  }, [searchParams])
  ...
  return (
    <div className="flex-1 bg-background">
      {/* Page Hero Header */}
      <section className="relative py-16 md:py-24 bg-gradient-to-b from-primary/10 via-transparent to-transparent border-b border-border/60">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                {contactInfo.map((info, index) => {
                  return (
                    <div key={index} className="flex gap-4 items-center">
                      <div className="w-12 h-12 rounded-full overflow-hidden border border-primary/20 flex-shrink-0 relative shadow-sm">
                        <Image
                          src={info.image}
                          alt={info.title}
                          fill
            ...
            <motion.div
              ...
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6 }}
              className="lg:col-span-7 bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm hover:border-primary/35 transition-all duration-300"
            >
              <h2 className="text-2xl font-bold text-foreground mb-2">
                Send a Message
              </h2>
              ...
              {submitted && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="mb-6 p-4 bg-accent/10 border border-accent/25 text-primary rounded-xl text-sm font-semibold flex items-center gap-2"
                >
                  <span>✓</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {greenhouseLocations.map((loc, i) => (
              <motion.div
                ...
                className="rounded-3xl bg-background border border-border/80 shadow-sm flex flex-col md:flex-row overflow-hidden hover:border-primary/30 transition-all duration-300 min-h-[220px]"
              >
                {/* Branch Image */}
                ...
                <div className="p-6 md:p-8 flex flex-col justify-between flex-1 space-y-4">
                  <div className="space-y-2">
                    <h3 className="text-primary font-bold text-lg font-serif">{loc.branch}</h3>
                  ...
                  <div className="space-y-1.5 border-t border-border/50 pt-4 text-xs font-medium text-neutral-600">
                    <p className="flex items-start gap-2 leading-relaxed">
                      <span className="text-primary-emerald font-semibold flex-shrink-0">Address:</span>
                      <span className="text-neutral-500 font-light">{loc.address}</span>
                    </p>
                    <p className="flex items-center gap-2">
                      <span className="text-primary-emerald font-semibold flex-shrink-0">Phone:</span>
  ...
  return (
    <main className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1">
        <Suspense fallback={
          <div className="max-w-7xl mx-auto px-4 py-24 text-center text-muted-foreground font-light text-sm">
