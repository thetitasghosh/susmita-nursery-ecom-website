...
import { Camera, Heart, ShoppingCart, RotateCcw, Play, CheckCircle2, Sparkles, MessageSquare, ArrowUp } from 'lucide-react'
...
export default function ARExperiencePage() {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: 'user' | 'expert', text: string }>>([
    { sender: 'expert', text: "Hello! I'm Susmita's senior horticulturist. Unsure which plant suits your humidity and lighting? Ask me anything!" }
  ])
  ...
  const scrollToVisualizer = () => {
    setIsCameraOn(true)
    visualizerSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      let reply = "That's a great choice! For that environment, we highly recommend plants like the Areca Palm or Snake Plant which are very resilient."
      if (userMsg.toLowerCase().includes('pet')) {
        reply = "If you have pets, the Areca Palm or Monstera (under supervision) are beautiful. The Areca Palm is 100% non-toxic and pet-safe!"
      } else if (userMsg.toLowerCase().includes('dark') || userMsg.toLowerCase().includes('low light')) {
        reply = "For low light settings, Aglaonema Red or the Snake Plant will thrive beautifully with minimal indirect sun."
      }
  ...
  const upsellPlants = allProducts.filter(p => [1, 13, 14, 2, 16, 3].includes(p.id))
  ...
  return (
    <main className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
            ...
            >
              ...
              <h1 className="text-4xl sm:text-6xl font-serif font-bold text-foreground leading-tight">
                See Plants In Your Space<br />Before You Buy.
              </h1>
              <p className="text-base sm:text-lg text-muted-foreground font-light leading-relaxed max-w-2xl mx-auto font-sans">
                Bring nature home with confidence. Use our interactive web AR experience to see how plants look, fit, and scale in your space in real time.
              </p>
            </motion.div>
        ...
        <section ref={visualizerSectionRef} className="py-16 md:py-24 scroll-mt-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              ...
              <div className="lg:col-span-7 space-y-6">
                <div
                  className="relative h-[400px] sm:h-[550px] rounded-[36px] overflow-hidden border border-border bg-neutral-950 shadow-2xl z-10 group"
                >
                  {/* Video feed */}
                  {isCameraOn && (
                    <video
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2 bg-neutral-950/90 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full text-xs font-semibold text-white shadow-sm z-20">
                    <span className={`w-2.5 h-2.5 rounded-full ${isCameraOn ? 'bg-secondary animate-pulse' : 'bg-accent'}`} />
                    <span>{isCameraOn ? 'Webcam Fitting Active' : 'Room Backdrop Mode'}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <Button
                      ...
                      className={`w-full sm:w-auto rounded-full px-6 font-semibold cursor-pointer py-5 ${
                        isCameraOn ? 'bg-orange-500 hover:bg-orange-600' : 'bg-primary hover:bg-primary-emerald'
                      }`}
                    >
                      <Camera size={16} className="mr-2" />
                      {isCameraOn ? 'Switch to Room backdrop' : 'Open Live Webcam'}
                    </Button>
                  </div>

                  {!isCameraOn && (
                    <div className="flex items-center gap-2 w-full sm:w-auto mt-3 sm:mt-0 overflow-x-auto pb-1 sm:pb-0">
                      <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider whitespace-nowrap">Templates:</span>
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="bg-card border border-border p-6 rounded-3xl shadow-sm space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Auto Rotate</span>
                    <button
                ...
                <div className="bg-primary/5 border border-primary/10 p-6 rounded-3xl space-y-4">
                  <div className="flex gap-4 items-center">
                    ...
                    <div>
                      <span className="text-[9px] font-sans font-bold text-primary tracking-widest uppercase block">{activePlant.category}</span>
                      <h4 className="font-serif font-bold text-foreground text-lg leading-tight">{activePlant.name}</h4>
                      {activePlant.scientificName && (
                        <p className="text-[10px] text-muted-foreground italic font-serif leading-none mt-0.5">{activePlant.scientificName}</p>
                      )}
                      <p className="text-sm font-serif font-bold text-primary mt-1">₹{activePlant.price.toFixed(2)}</p>
                    </div>
                  </div>

                  <p className="text-xs text-muted-foreground font-light leading-relaxed">
                    Satisfied with how the {activePlant.name} projects in your room? Add this specimen directly to your cart or save it for later.
                  </p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className={`w-12 rounded-full border border-border flex items-center justify-center cursor-pointer transition-colors ${
                        wishlisted ? 'bg-primary/10 border-primary text-primary hover:bg-primary/20' : 'hover:bg-muted text-foreground'
                      }`}
                    >
                ...
                <div className="bg-accent/5 border border-accent/25 p-6 rounded-3xl space-y-4">
                  ...
                  <ul className="space-y-3 text-xs text-neutral-600 font-light font-sans">
                    <li className="flex items-start gap-2.5">
                      ...
                      <div>
                        <strong className="font-semibold text-foreground">Realistic Preview</strong> — Inspect high-fidelity leaves and textures in real sizes.
                      </div>
                    </li>
                    <li className="flex items-start gap-2.5">
                      <CheckCircle2 size={14} className="text-primary mt-0.5 flex-shrink-0" />
                      <div>
                        <strong className="font-semibold text-foreground">Perfect Fit</strong> — Validate vertical heights and widths in your rooms before shipping.
                      </div>
                    </li>
                    <li className="flex items-start gap-2.5">
                      <CheckCircle2 size={14} className="text-primary mt-0.5 flex-shrink-0" />
                      <div>
                        <strong className="font-semibold text-foreground">Any Space Usable</strong> — Works in living rooms, study desks, patio spaces, and balconies.
                      </div>
                    </li>
                    <li className="flex items-start gap-2.5">
                      <CheckCircle2 size={14} className="text-primary mt-0.5 flex-shrink-0" />
                      <div>
                        <strong className="font-semibold text-foreground">Better Decisions</strong> — No more guessing if a plant fits or suits the aesthetics.
                      </div>
                    </li>
                  </ul>
                </div>
        ...
        <section className="py-20 bg-white border-t border-b border-border/40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
              {[
                { step: '1', title: 'Open Camera', desc: 'Tap on "Start AR Experience" to open your webcam or phone camera.' },
                { step: '2', title: 'Scan Your Space', desc: 'Move your camera around slightly to detect flat floor surfaces.' },
                { step: '3', title: 'Choose a Plant', desc: 'Select any plant variety from our dynamic spec grid below.' },
                { step: '4', title: 'View in AR', desc: 'See the plant model projected in its real size and shape.' },
                { step: '5', title: 'Make It Yours', desc: 'Love the styling? Click "Add to Cart" to bring it home!' }
              ].map((item, idx) => (
                <div key={idx} className="space-y-4 flex flex-col items-center p-4">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            <div className="text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground font-light max-w-md mx-auto">
                Tap on any popular houseplant below to immediately project it in the simulator box above.
              </p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-6">
              {upsellPlants.map((plant) => {
                const isSelected = allProducts[selectedPlantIndex]?.id === plant.id
                return (
                  <motion.div
                    key={plant.id}
                    whileHover={{ y: -4 }}
                    onClick={() => {
                      const actualIdx = allProducts.findIndex(p => p.id === plant.id)
                      if (actualIdx > -1) {
                    ...
                    className={`relative rounded-3xl overflow-hidden border cursor-pointer bg-card transition-all duration-300 flex flex-col justify-between p-4 ${
                      isSelected ? 'border-primary shadow-md ring-2 ring-primary/20' : 'border-border hover:border-primary/30 shadow-sm'
                    }`}
                  >
                    <div>
                      {/* Image */}
                      ...
                      <span className="text-[8px] font-sans font-bold text-primary uppercase tracking-widest">{plant.category}</span>
                      <h4 className="font-serif font-bold text-foreground text-sm line-clamp-1 mt-0.5">{plant.name}</h4>
                      {plant.scientificName && (
                        <p className="text-[9px] text-muted-foreground italic font-serif leading-none mt-0.5">{plant.scientificName}</p>
                      )}
                    </div>

                    <div className="flex items-center justify-between mt-3.5 pt-2 border-t border-border/40">
                      <span className="font-serif font-bold text-primary text-xs tabular-nums">₹{plant.price}</span>
                      <button className={`p-1.5 rounded-full cursor-pointer transition-colors ${isSelected ? 'bg-primary text-white' : 'bg-muted text-foreground hover:bg-primary hover:text-white'}`}>
                        <ArrowUp size={10} />
                      </button>
                    </div>
                  </motion.div>
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-[36px] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
              <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                ...
                <div className="space-y-1.5">
                  <span className="text-[8px] font-sans font-bold tracking-widest text-accent uppercase block">Free Consultancy</span>
                  <h3 className="font-serif font-bold text-xl sm:text-2xl">Not sure which plant to choose?</h3>
                  <p className="text-xs text-neutral-300 font-light max-w-sm leading-relaxed">
                    Our Nadia greenhouse specialists are online to verify your spatial light, humidity, and pet-friendly concerns.
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <div className="aspect-video w-full rounded-2xl overflow-hidden border bg-black relative flex items-center justify-center">
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent z-10 flex flex-col justify-end p-6 text-white text-left">
                  <p className="font-serif font-bold text-lg mb-1">Fitting a Monstera in AR</p>
                  <p className="text-xs text-neutral-300 font-light">Follow the 3D projection dots to verify spatial placement.</p>
                </div>
              ...
              <div className="flex justify-end pt-2">
                <Button onClick={() => setVideoModalOpen(false)} className="rounded-full bg-primary">
                  Got It, Let&apos;s Fit!
                </Button>
              </div>
            </motion.div>
          ...
          >
            ...
            <div className="bg-primary-deep p-4 flex items-center justify-between text-white border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-serif font-semibold text-xs leading-none">Susmita Expert</h4>
                  <span className="text-[7px] text-secondary font-sans font-bold tracking-widest uppercase">Online Now</span>
                </div>
              </div>
              <button 
